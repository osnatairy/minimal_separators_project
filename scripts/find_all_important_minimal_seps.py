...
def _compute_xy_separators(pair):
    ...
    important = find_important_seps_for_pair(
        _WORKER_G,
        X,
        Y,
        "smallminimalseps",
...
def write_xy_to_zsets_csv(xy_to_zsets, filename):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["seed", "X", "Y", "len_z", "important_z"])

        for (seed, X, Y), important in sorted(xy_to_zsets.items()):
            for z in important:
                writer.writerow([
                    seed,
                    X,
    ...
    for node in nodes:
        for prob_node in prob_nodes:
            for k_roots in [int(node * 0.3)]:  # [node, int(node*0.3), 3, 1]:
                ...
                write_xy_to_zsets_csv(all_xy_to_zsets, f"important_seps_up/important_seps{variance}.csv")
