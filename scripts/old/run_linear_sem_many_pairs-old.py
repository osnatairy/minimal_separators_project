...
from sem.variance import example_compute_avar, conditional_variance_from_cov,sigma_from_sem, avar_henckel_single_xy,compute_avar_many_Z,compute_avar_many_Z_parallel#x_drain_two_sets
...
if __name__ == "__main__":
    ...
    N = 5
    nodes = [50]#,30,40, 50]
    prob_nodes =[0.2]#[0.07, 0.1, 0.15, 0.2, 0.3]
    betas = [0.7]
        ...
        for prob_node in prob_nodes:
            for k_roots in [int(node*0.3)]:#[node, int(node*0.3), 3, 1]:
                ...
                for seed in range(N):
                    ...
                    for X, Y in pairs:
                        H, Z_sets, closest = find_adjustment_sets_for_pair(sem.G, X, Y,"smallminimalseps", R=R, I=I, get_closest_seps=True)
