...
from pipelines.adjust_sets import find_closest_seps_for_pair, find_important_seps_for_pair
...
def _compute_xy_separators(pair):
    ...
    o_x_y_seperator = optimal_adjustment_set_O(_WORKER_G, X, Y)

    xy_key = (_WORKER_SEED, X, Y)

    return xy_key, o_x_y_seperator
...
def write_xy_to_zsets_csv(xy_to_zsets, filename):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["seed", "X", "Y", "len_z", "hankel_z"])

        for (seed, X, Y), hankel in sorted(xy_to_zsets.items()):

            writer.writerow([
                seed,
                X,
                Y,
                len(hankel),  # כמה מפרידים יש סה"כ
                list(hankel)  # מפריד אחד בלבד
            ])
    ...
    for node in nodes:
        for prob_node in prob_nodes:
            for k_roots in [int(node * 0.3)]:  # [node, int(node*0.3), 3, 1]:
                ...
                write_xy_to_zsets_csv(all_xy_to_zsets, f"important_seps/hankel_seps{variance}.csv")
