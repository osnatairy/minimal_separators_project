...
from causal.policies import PolicyFn, static_do_policy
...
_WORKER_A_NAME = None
_WORKER_L_VARS = None
_WORKER_POLICY_FN = None
_WORKER_VALUE_MAP = None
_WORKER_POLICY_FN = None
_WORKER_A_STAR = None
...
def _init_variance_worker(
    bn: BN,
    Y: str,
    A_name: str,
    L_vars: List[str],
    a_star,
    value_map: Dict[Any, float] | None,
):
    """
    פונקציית אתחול שרצה פעם אחת בכל worker process.
    בונה infer פעם אחת ושומרת הכל במשתנים גלובליים של אותו worker.
    """
    global _WORKER_BN
    global _WORKER_INFER
    global _WORKER_Y
    global _WORKER_A_NAME
    global _WORKER_L_VARS
    global _WORKER_POLICY_FN
    global _WORKER_A_STAR
    global _WORKER_VALUE_MAP

    _WORKER_BN = bn
    _WORKER_Y = Y
    _WORKER_A_NAME = A_name
    _WORKER_L_VARS = L_vars
    _WORKER_A_STAR = a_star
    _WORKER_VALUE_MAP = value_map

    _WORKER_POLICY_FN = static_do_policy(a_star=_WORKER_A_STAR)

    _, _WORKER_INFER = bn_to_pgmpy_model(bn)


def _variance_for_one_Z_worker(Z: List[str]) -> Tuple[Tuple[str, ...], float]:
    """
    מחשבת את השונות עבור קבוצת Z אחת, תוך שימוש
    ב-BN וב-infer שכבר נטענו פעם אחת ב-worker.
    """
    Z_key = tuple(sorted(Z))
    ...
    sigma2 = asymptotic_variance_for_Z(
        ...
        infer=_WORKER_INFER,
        Y=_WORKER_Y,
        A_name=_WORKER_A_NAME,
        Z_vars=Z,
        L_vars=_WORKER_L_VARS,
        policy_fn=_WORKER_POLICY_FN,
        value_map=_WORKER_VALUE_MAP,
    )

    return Z_key, sigma2


def asymptotic_variance_over_Z_sets_parallel(
    bn: BN,
    Y: str,
    A_name: str,
    Z_sets: List[List[str]],
    ...
    """
    מחשבת σ^2_{π,Z}(P) עבור כמה קבוצות התאמה Z שונות במקביל.

    בכל worker בונים infer פעם אחת בלבד.
    """
    ...
    valid_Z_sets = [Z for Z in Z_sets if len(Z) >= 1]

    with ProcessPoolExecutor(
        max_workers=max_workers,
        initializer=_init_variance_worker,
        initargs=(bn, Y, A_name, L_vars, a_star, value_map),
    ) as executor:
        futures = [executor.submit(_variance_for_one_Z_worker, Z) for Z in valid_Z_sets]

        for future in as_completed(futures):
            Z_key, sigma2 = future.result()
            results[Z_key] = sigma2
